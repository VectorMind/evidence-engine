...
def render_summary_markdown(payload: dict[str, Any]) -> str:
    ...
    if payload.get("command") == "index folder":
...
<main>
  <h1>{html.escape(title)}</h1>
  <p class="muted">Generated by agents-docs from a structured parse result.</p>
  <h2>Overview</h2>
  {overview}
...
def _scan_console_summary(payload: dict[str, Any]) -> str:
    ...
    statistics = payload.get("statistics", {})
    lines = [
        f"scan folder: {payload.get('status', 'unknown')} ({payload.get('root_label', 'folder')})",
        (
...
def _parse_console_summary(payload: dict[str, Any]) -> str:
    counts = payload.get("counts", {})
    lines = [
        f"parse folder: {payload.get('status', 'unknown')} ({payload.get('root_label', 'folder')})",
        (
...
def _index_console_summary(payload: dict[str, Any]) -> str:
    ...
    lines = [
        f"index folder: {payload.get('status', 'unknown')} ({payload.get('root_label', 'folder')})",
        (
...
def _health_console_summary(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            f"cache {payload.get('cache_root', 'n/a')}",
